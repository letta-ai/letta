...
- `relevance` - Scoring information (`rrf_score`, `vector_rank`, `fts_rank`)
...
**Note:**
- Agents cannot control insertion timestamps - memories are timestamped automatically
- Developers can backdate memories via SDK by providing a `created_at` timestamp
- Agents know the current day but not the current time
- Agents can see timestamps of messages they've received
- Time filtering is useful for queries like "what did we discuss last week?"
