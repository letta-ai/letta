...
type UpdateEmbeddingModelFormValues = z.infer<typeof updateEmbeddingModelSchema>

function UpdateEmbeddingModelForm(props: UpdateEmbeddingModelFormProps) {
  const { model } = props;
  const queryClient = useQueryClient();
  const { mutate, isPending } = webApi.admin.models.updateAdminEmbeddingModel.useMutation({
    onSuccess: () => {
      void queryClient.invalidateQueries({
        queryKey: queryClientKeys.admin.models.getAdminEmbeddingModelById(model.id),
      });
      void queryClient.invalidateQueries({
  ...
  const handleSubmit = useCallback((values: UpdateEmbeddingModelFormValues) => {
    mutate({
      params: {
        id: model.id,
      },
...
function EmbeddingModelPage(props: EmbeddingModelPageProps) {
  ...
  const { data: model, isLoading, isError } = webApi.admin.models.getAdminEmbeddingModel.useQuery({
