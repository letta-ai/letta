...
type SyncToolsWithComposioContractResponse = ServerInferResponses<typeof contracts.admin.tools.syncToolsWithComposioContract>;
...
async function syncToolsWithComposio(): Promise<SyncToolsWithComposioContractResponse> {
  ...
  const existingTools = await db.query.toolMetadata.findMany({
    where: and(
      ...
      inArray(toolMetadata.providerId, composioTools.map(t => t.enum)),
    )
  })
  ...
  await Promise.all(existingTools.map(t => {
    ...
    return db.update(toolMetadata).set({
  ...
  await Promise.all(newTools.map(t => {
  ...
  const newBrands = Array.from(uniqueBrands).filter(v => !existingBrandSet.has(v));
  ...
  await Promise.all(
    existingUniqueBrands.map((brand) => {
      return db.update(toolGroupMetadata).set({
        imageUrl: uniqueBrandsToImage.get(brand.brand),
      }).where(eq(
        toolGroupMetadata.brand,
        brand.brand
      ))
    })
  )
