...
The `compose.yaml` file will read your local environment variables and pass them to the Letta server running in the container. To use Google AI with the Letta server, set the `GEMINI_API_KEY` environment variable and run the server with `docker compose up`:
```bash
export GEMINI_API_KEY="..."
docker compose up 
```
