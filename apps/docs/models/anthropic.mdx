...
The `compose.yaml` file will read your local enviornment variables and pass them to the Letta server running in the container. To use Anthropic with the Letta server, set the `ANTHROPIC_API_KEY` enviornment variable and run the server with `docker compose up`:
```bash
export ANTHROPIC_API_KEY="sk-ant-..." 
docker compose up 
```
