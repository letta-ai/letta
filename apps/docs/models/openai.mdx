...
You can use Letta with OpenAI if you have an OpenAI account and API key. Once you have set your `OPENAI_API_KEY` in your enviornment variables, you can select what model and configure the context window size.
...
The `compose.yaml` file will read your local enviornment variables and pass them to the Letta server running in the container. To use OpenAI with the Letta server, set the `OPENAI_API_KEY` enviornment variable and run the server with `docker compose up`:
```bash
export OPENAI_API_KEY="sk-..." 
docker compose up 
```
