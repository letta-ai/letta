...
<Note>
Letta Desktop is in **alpha**. View known issues [here](/guides/desktop/troubleshooting).<br />
For bug reports and feature requests, please [join our Discord](https://discord.gg/letta).
</Note>

## Install Letta Desktop
You can install Letta Desktop for MacOS (M series) or Windows on [our install page](/install).
<img className="w-300" src="/images/letta_desktop_screenshot.png" />

If you don't have an M series (Apple Silicon) Mac or a Window machine (for example if you are using Linux), you can still use [Letta via Docker](/quickstart/docker) or [pip](/guides/server/pip).
