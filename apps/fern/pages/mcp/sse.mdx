...
To mount your `~/.letta/mcp_config.json` file into Docker, when we run `docker run` we need to add `-v ~/.letta/mcp_config.json:/app/config/mcp_config.json`:
```bash
docker run \
  -v ~/.letta/.persist/pgdata:/var/lib/postgresql/data \
  -v ~/.letta/mcp_config.json:/app/config/mcp_config.json \
  -p 8283:8283 \
  --env-file .env \
  letta/letta:latest
```
