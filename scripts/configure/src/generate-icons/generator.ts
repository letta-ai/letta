...
function generateIndexFile(icons: string[]) {
  return icons
    .map((icon) => `export * from './_autogenerated/${icon}';`)
    .join('\n');
}
...
export async function generateIconsGenerator(
  ...
  const existingIconsAsSet = new Set(
    existingIcons.map((icon) => icon.replace('.tsx', ''))
  );

  const iconsToGenerate = regenerate
    ? allIcons
