...
export async function generateIconsGenerator(
  ...
  const iconsToGenerate = fs.readdirSync(
    path.join(workspaceRoot, 'libs/ui-component-library/src/lib/icons/sources')
  );
  ...
  iconsToGenerate.forEach((fileName) => {
    const svgContent = tree
      .read(path.join('libs/ui-component-library/src/lib/icons/sources', fileName))
      ?.toString();
    ...
    // regex to camelcase attributes
    // .replace(/[A-z](-[A-z]+)=/g, x=>x[1].toUpperCase())
    tree.write(
      path.join(
        'libs/ui-component-library/src/lib/icons/_autogenerated',
        `${componentName}.tsx`
      ),
      componentContent
    );
