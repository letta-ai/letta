...
export async function setupGenerator(
  ...
  generateFiles(tree, path.join(__dirname, 'files'), path.join('.'), {
    ...
    GOOGLE_CLIENT_ID:
      options.GOOGLE_CLIENT_ID || LOCAL_DEFAULT_OPTIONS.GOOGLE_CLIENT_ID,
    GOOGLE_CLIENT_SECRET:
      options.GOOGLE_CLIENT_SECRET ||
      LOCAL_DEFAULT_OPTIONS.GOOGLE_CLIENT_SECRET,
    GOOGLE_REDIRECT_URI:
      options.GOOGLE_REDIRECT_URI || LOCAL_DEFAULT_OPTIONS.GOOGLE_REDIRECT_URI,
    REDIS_HOST: options.REDIS_HOST || LOCAL_DEFAULT_OPTIONS.REDIS_HOST,
    REDIS_PASSWORD:
      options.REDIS_PASSWORD || LOCAL_DEFAULT_OPTIONS.REDIS_PASSWORD,
    REDIS_PORT: options.REDIS_PORT || LOCAL_DEFAULT_OPTIONS.REDIS_PORT,
    LETTA_AGENTS_ENDPOINT:
      options.LETTA_AGENTS_ENDPOINT ||
      LOCAL_DEFAULT_OPTIONS.LETTA_AGENTS_ENDPOINT,
    NEXT_PUBLIC_CURRENT_HOST:
      options.NEXT_PUBLIC_CURRENT_HOST ||
      LOCAL_DEFAULT_OPTIONS.NEXT_PUBLIC_CURRENT_HOST,
