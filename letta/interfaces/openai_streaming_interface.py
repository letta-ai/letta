...
class SimpleOpenAIStreamingInterface:
    ...
    def __init__(
        ...
        # For DeepSeek Reasoner: buffer content to detect JSON function calls
        self.content_buffer = ""  # Accumulate content chunks
    ...
    async def _process_chunk(
        ...
        if chunk.choices:
            ...
            if message_delta.content is not None and message_delta.content != "":
                # Add to buffer
                self.content_buffer += message_delta.content
                
                # Check if we're starting JSON buffering (single object or array)
                stripped = self.content_buffer.strip()
                ...
                if self.is_buffering_json:
                    is_complete, parsed = self._try_parse_json_function_call(self.content_buffer)
                    
                    if is_complete:
                        if parsed:
                            # Handle both single function call (dict) and parallel calls (list)
                            # For now, only process the first call (Letta streaming doesn't fully support parallel yet)
                            function_call_data = parsed[0] if isinstance(parsed, list) else parsed
                            
                            # It's a function call! Convert to tool call
                            ...
                            # Set the streaming interface state so the agent knows to execute the tool
                            # These fields are checked by the agent loop after streaming completes
                            self.tool_call_name = function_call_data["name"]
                            self.tool_call_args = arguments_str
                            self.tool_call_id = get_tool_call_id()
                            
                            # Create tool call using Letta's ToolCall class (already imported)
                            ...
                            # Create ToolCallMessage (already imported at top)
                            tool_call_msg = ToolCallMessage(
                                id=self.letta_message_id,
                                date=datetime.now(timezone.utc),
                                otid=Message.generate_otid_from_id(self.letta_message_id, message_index),
                            ...
                            # It's JSON but not a function call, treat as regular content
                            assistant_msg = AssistantMessage(
                                id=self.letta_message_id,
                                content=[TextContent(text=self.content_buffer)],
                                date=datetime.now(timezone.utc).isoformat(),
                                otid=Message.generate_otid_from_id(self.letta_message_id, message_index),
                                run_id=self.run_id,
                                step_id=self.step_id,
