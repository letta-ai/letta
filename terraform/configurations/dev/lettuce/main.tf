terraform {
  backend "gcs" {
    bucket = "memgpt-tf"
    prefix = "infra/tfstate/dev/lettuce"
  }
...
# Create a dedicated GCP service account for lettuce
resource "google_service_account" "lettuce_sa" {
...
module "k8s-iam" {
  ...
  sa_email   = google_service_account.lettuce_sa.email
  sa_name    = google_service_account.lettuce_sa.name
