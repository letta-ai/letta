terraform {
  backend "gcs" {
    bucket = "memgpt-tf"
    prefix = "infra/tfstate/dev/cloud-api"
  }
...
# Create a dedicated GCP service account for cloud-api
resource "google_service_account" "cloud_api_sa" {
...
module "k8s-iam" {
  ...
  sa_email   = google_service_account.cloud_api_sa.email
  sa_name    = google_service_account.cloud_api_sa.name
