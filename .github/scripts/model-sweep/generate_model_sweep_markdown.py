...
def process_global_models_config(input_file: str, output_file: str) -> None:
    ...
    sorted_providers = sorted(
        providers.keys(),
        key=lambda x: (-provider_scores[x], x)
    )
    ...
    max_context_width = max((len(f"{model['context_window']:,}") for provider_models in providers.values() for model in provider_models), default=10)
    ...
    for provider_name in sorted_providers:
        ...
        header_parts.extend([
            f"{'Context Window':^{max_context_width}}",
            f"{'Last Scanned':^12}"
        ])
        ...
        for model in provider_models:
            ...
            row_parts.extend([
                f"{model['context_window']:,}".center(max_context_width),
                f"{model['last_scanned']}".center(12)
            ])
            row = "| " + " | ".join(row_parts) + " |"
