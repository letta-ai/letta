...
interface DisableHITLForToolDialogProps {
  trigger: React.ReactNode;
  toolName: string;
}

export function DisableHITLForToolDialog(props: DisableHITLForToolDialogProps) {
  const { trigger, toolName } = props;
  const { id: agentId } = useCurrentAgent();
  const t = useTranslations('ADE/Tools.DisableHITLForToolDialog');
    ...
    onMutate: () => {
      ...
      queryClient.setQueriesData<AgentState | undefined>(
        ...
        (oldData) => {
          ...
          return {
            ...oldData,
            tool_rules: (oldData.tool_rules || []).filter((rule) => {
              if (
