...
type StopReasonType = 
  'cancelled' | 'end_turn' | 'error' | 'invalid_tool_call' | 'max_steps' | 'no_tool_call' | 'tool_rule';
...
export function StopReason(props: StopReasonProps) {
  const t = useTranslations(
    'components/MessageReplay/StopReason',
  );
  ...
  function getStopReasonBadgeVariant(stopReason: string): 'default' |'destructive' | 'success' | 'warning' {
  switch (stopReason) {
    case 'end_turn':
    case 'tool_rule':
      return 'success';
