...
FOLDER_NAME = "Letta File Example"
...
# Files can be replaced if their content changes as well:
client.folders.files.upload(
    folder_id=folder_id,
    file=file_object,
    duplicate_handling="replace" # use this to overwrite existing files
)

# You can upload files using a separate name:
file = client.folders.files.upload(
    folder_id=folder_id,
    file=file_object,
    name="/something/example.txt",
    duplicate_handling="skip"
)
...
print(f"Access this agent at https://app.letta.com/agents/{agent.id}")
